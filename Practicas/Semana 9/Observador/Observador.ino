...
// ----- Calcular ŷk = Cd * x̂k + Dd * uk -----
  y_estimado = C_d[0][0] * estados[0] + C_d[0][1] * estados[1] + D_d[0][0] * u;
...
//----- Calcular x̂(k+1) = Ad*x̂ + L*e + Bd*u -----
  ...
  //estados_sig[0] = estados[0] + 0.02*estados[1] + 0.4520*e;
  estados_sig[1] = A_d[1][0]*estados[0] + A_d[1][1]*estados[1] + B_d[1][0]*(u) + L[1]*e;
  //estados_sig[1] = -2.3124*estados[0] + 0.2065*estados[1] + 2.3088*(u) + -7.0249*e;
...
// ----- Actualizar estado -----
  ...
  datos[3] = g.gyro.x *(-180/PI); //tita_punto medido
